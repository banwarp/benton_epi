...
###### Addition to _us
# abbr. Identifies the state
######



acs_data_extraction_us <- function(lookup_table_name, geo_vector, geo_names, agg_vector,granularity,year_folder,abbr) {
  ...
  ########## One change from acs_data_extraction.R #############
  ########## replace 'or' with correct abbreviation ############
  ##########
  lookup_table[,'table_name'] <- gsub('or',abbr,lookup_table[,'table_name'])
  ##########
  ...
  for(i in rel_data) {
    ...
    if(data_table_name != current_table_name) {
      data_table <- fread(data_table_name, sep=",", header=FALSE,data.table=FALSE)
