...
source("pts_funScript5.2.2020.R")
...
parmList = list(
                ...
                "simID" = "covid5.2.2020.1",            # Simulation ID
                ...
                "I0Pop" = 40,                              # Initial number of infected
                "maxINodeProp" = 1/10,                     # Maximum proportion of nodes that intially have one or more infected
                ...
                "sIProp" = .001,                           # Proportion of students who are infected; R, Im, and M are calculated
                ### gdata parameters
                # disease dynamics
                ...
                "parachuteNum" = 1,                        # Number of infected in each parachute event
...
N <- min(500,parmList$N) # capping number of nodes per trial
...
I0Pop <- parmList$I0Pop  # population that is infected in intial state
maxINodeProp <- parmList$maxINodeProp # proportion of nodes with one or more infected in initial state
...
sIProp <- parmList$sIProp # proportion of students who are infected
...
transitions <- c(
  ...
  "S -> (1/phi)*beta*betaRandomizer*(season)*S*I/(S+E+I+R+Im) -> E",      # Exposures, allows for effective or ineffective policies and seasonal variation in beta
...
# local parameters
lparms <- data.frame(
  # betaRandomizer = rep(runif(numTrials,min=.9,max=1.1),each=N) # randomizer for beta,
)

# diffusion matrix - part of ldata
...
v0 = data.frame(
  ...
  betaRandomizer = rep(runif(numTrials,min=.9,max=1.1),each=N), # randomizer for beta or R0
