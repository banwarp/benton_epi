...
### add option to cluster nodes (higher transferRate within nodes compared to across nodes)
...
covidWrapper <- function(
  ...
  transferNodeNum = .05,                   # Average number/proportion of nodes that transfer at each transfer event
  transferMinProp = .1,                    # Minimum proportion of node population that transfers in each event
  transferMaxProp = .3,                    # Maximum proportion of node population that transfers in each event
  ...
  source("massEventFunction5.8.2020.R")
  source("simInfPlottingFunction5.8.2020.R")
  ...
  if(transferNodeNum<1) {transferNodeNum <- ceiling(transferNodeNum*N)} # converts proportion to number
  numTransferList <- lapply(1:numTrials,function(x) rpois(1,transferRate*transferNodeNum*max(tspan)))
  
  # changing node=dest transfers to different dest
  tfun <- function(n=node,t=trial,ntm=nodeTrialMat){
    trialNodes <- nodeTrialMat[which(nodeTrialMat$trial==t),(node)]
  ...
  if(max(unlist(numTransferList))>0) {
    ...
    transferSame <- merge(transferSame,nodeTrialMat,by="node")
    transferSame <- transferSame[order(rn)]
    newDest <- transferSame[,tfun(node,trial),by=seq_len(nrow(transferSame))]
  ...
  # shift data frame
  Ndf <- matrix(c(1,rep(0,length(compartments)-1)),ncol=1)
  dimnames(Ndf) <- list(compartments,1)
  ...
  if(nrow(allEvents)>0) {
    model <- mparse(transitions = transitions, compartments = compartments, events = allEvents, E=E,N=Ndf,
