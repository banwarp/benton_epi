# covid19_SimInf_ageGroups03.07.2023.R
...
# Added in functionality to use kmeans clustering to group different simulated trajectories
...
covidWrapper <- function(
  ...
  folderPath = "C:/Users/peter/OneDrive/Documents/OSU/Research/covidScripts",                       # folder path for subroutines and output
  ...
  kmeansClustering = TRUE,                # logical for kmeans clustering of trajectories
  kmeansCompts= "cumIk_cumIy_cumIa_cumIs", # compartments for kmeans clustering and central tendency
  kmeansCenters = 4,                    # number of kmeans clusters
  ...
  includeCentralTendencyCluster = TRUE,   # whether to include a central tendency for each cluster according to the cumulative norm
  separateClusters = TRUE, # whether to graph clusters separately and return as a list of plots
  ...
  lString = "Cluster",                      # Title of legend. If plotting clusters: "Cluster".
  clString = "Cluster",                         # plot parameter: Title of legend
  slString = "Relative likelihood",    # plot parameter: Title of legend
  ...
  source("simInfPlottingFunction_03.05.2023.R")
  ...
  trajPlotInfections <- simInfPlottingFunction(
    ...
    compts = "hIk_hIy_hIa_hIs_lIk_lIy_lIa_lIs_uhIk_uhIy_uhIa_uhIs_ulIk_ulIy_ulIa_ulIs", # compartments to sum and plot
    newI = FALSE,                              # whether to plot new infections
    ...
    kmeansClustering = kmeansClustering,       # logical for kmeans clustering of trajectories
    kmeansCompts= "cumIk_cumIy_cumIa_cumIs",   # compartments for kmeans clustering and central tendency
    kmeansCenters = kmeansCenters,       # number of kmeans clusters
    includeCentralTendency = includeCentralTendency, # whether to include a single trajectory that is the central tendency according to the cumulative norm
    includeCentralTendencyCluster = includeCentralTendencyCluster, # whether to include a central tendency for each cluster according to the cumulative norm
    separateClusters = separateClusters, # whether to graph clusters separately and return as a list of plots
    ...
    includeMedian = FALSE,                      # Plot the median
    ...
    byHour = FALSE,                            # whether to change timescale to hours (useful for plotting school data)
    dateBreaks = dateBreaks,                   # plot parameter: Date axis format
    dataTitle = "Infections",                  # plot parameter: title of the data
    ...
    clString = clString,                         # plot parameter: Title of legend
    slString = slString,
  ...
  trajPlotPhi <- simInfPlottingFunction(
    ...
    clString = "Cluster",                         # plot parameter: Title of color legend
    slString = "Relative likelihood",             # plot parameter: Title of size legend
    cString = cString                          # plot parameter: Plot caption
  )
  
  # Warning message:
  #   In as.data.table.list(x, keep.rownames = keep.rownames, check.names = check.names,  :
  #                           Item 2 has 35900 rows but longest item has 36500; recycled with remainder.
  # This is because of the rolling average
  ...
  if(hospRateExp > 0 | hospRatePost > 0){
    trajPlotHosp <- simInfPlottingFunction(
      ...
      clString = clString,                       # plot parameter: Title of color legend
      slString = slString,                       # plot parameter: Title of size legend
      cString = cString                          # plot parameter: Plot caption
    )
  }
  ...
  trajPlotcumI <- simInfPlottingFunction(
    ...
    table = "U",                               # which table: U or V
    compts= "cumIk_cumIy_cumIa_cumIs",         # compartments that will be plotted
    newI = FALSE,                              # logical for new infections
    groups = plotGroups,                       # List of groups to aggregate and plot
    sumGroups = sumGroups,                     # Automatically sums over all groups
    uNames = names(u0),                        # list of compartments
    vNames = NULL,                             # list of variables
    ...
    startDate = startofSimDay,                 # start date of simulation
    dateBreaks = dateBreaks,                   # plot parameter: Date axis format
    dataTitle = "Cumulative infections",       # title of the data
    titleString = paste0("Daily Cumulative Infections in ",titleString),        # plot parameter: Title of plot
    xString = "Date",                          # plot parameter: Title of x axis
    yString = "Number of infections",          # plot parameter: Title of y axis
    clString = clString,                       # plot parameter: Title of color legend
    slString = slString,                       # plot parameter: Title of size legend
  ...
  if(separateClusters){
    combinedInfections <- trajPlotInfections[[1]]
    separateInfections <- do.call("grid.arrange", c(trajPlotInfections[[2]], ncol=floor(sqrt(length(trajPlotInfections[[2]])))))
    
    combinedPhi <- trajPlotPhi[[1]]
    separatePhi <- do.call("grid.arrange", c(trajPlotPhi[[2]], ncol=floor(sqrt(length(trajPlotPhi[[2]])))))
    
    combinednewI <- trajPlotnewI[[1]]
    separatenewI <- do.call("grid.arrange", c(trajPlotnewI[[2]], ncol=floor(sqrt(length(trajPlotnewI[[2]])))))
    
    combinedcumI <- trajPlotcumI[[1]]
    separatecumI <- do.call("grid.arrange", c(trajPlotcumI[[2]], ncol=floor(sqrt(length(trajPlotcumI[[2]])))))
    
    ggsave(paste0(plotName,"ICombined.png"),combinedInfections,width=9,height=5,units="in")
    ggsave(paste0(plotName,"ISeparate.png"),separateInfections,width=13.5,height=7.5,units="in")
    ggsave(paste0(plotName,"phiCombined.png"),combinedPhi,width=9,height=5,units="in")
    ggsave(paste0(plotName,"phiSeparate.png"),separatePhi,width=13.5,height=7.5,units="in")
    ggsave(paste0(plotName,"newICombined.png"),combinednewI,width=9,height=5,units="in")
    ggsave(paste0(plotName,"newISeparate.png"),separatenewI,width=13.5,height=7.5,units="in")
    ggsave(paste0(plotName,"cumICombined.png"),combinedcumI,width=9,height=5,units="in")
    ggsave(paste0(plotName,"cumISeparate.png"),separatecumI,width=13.5,height=7.5,units="in")
    
    if(hospRateExp > 0 | hospRatePost > 0){
      combinedHosp <- trajPlotHosp[[1]]
      separateHosp <- do.call("grid.arrange", c(trajPlotHosp[[2]], ncol=floor(sqrt(length(trajPlotHosp[[2]])))))
      ggsave(paste0(plotName,"hospCombined.png"),combinedHosp,width=9,height=5,units="in")
      ggsave(paste0(plotName,"hospSeparate.png"),separateHosp,width=13.5,height=7.5,units="in")
    }
    
    if(hospDeathRate > 0 | nonHospDeathRate > 0){
      combinedDeaths <- trajPlotDeaths[[1]]
      separateDeaths <- do.call("grid.arrange", c(trajPlotDeaths[[2]], ncol=floor(sqrt(length(trajPlotDeaths[[2]])))))
      ggsave(paste0(plotName,"deathsCombined.png"),combinedDeaths,width=9,height=5,units="in")
      ggsave(paste0(plotName,"deathsSeparate.png"),separateDeaths,width=13.5,height=7.5,units="in")
    }
