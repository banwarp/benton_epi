...
covidWrapper <- function(
  ...
  mIProp = .001,                           # Proportion of individuals who are infectious
  muIProp = 0,                             # Proportion of individuals who are unknown-infectious; other compartments are calculated
  ...
  source("pts_funScript06.16.2020.R")
  ...
  compartments <- c("S",        # Susceptible
                    "E",        # Exposed
                    "hI",       # initial high spreader Infectious (before success/failure of monitoring)
                    "lI",       # initial low spreader Infectious (before success/failure of monitoring)
                    "uhI",      # unknown high spreader Infectious (after failure of monitoring)
                    "ulI",      # unknown low spreader Infectious (after failure of monitoring)
                    "R",        # Recovered
                    "Im",       # Immune
  ...
  gdata = data.frame(
    ...
    spreadRatio = highSpreadRate/(1-highSpreadRate),   # Ratio of transmissibility of low spreaders to high spreaders
    ...
    monitoringSuccess = monitoringSuccess,             # How successful symptom monitoring is to transition from hI/lI to pI
  ...
  # "S -> (betaI*((1/phi)*(spreadRatio*hI + (1/spreadRatio)*lI))+   # Exposures: Exposure rate among initial infectious
  #        betaP*pI+                                                # Exposure rate among post infectious; does not depend on phi
  #        betaU*((1/phi)*(spreadRatio*uhI+(1/spreadRatio)*ulI)))*  # Exposure rate among unknown infectious
  #        season*betaRandomizer*                                   # Seasonal effect and randomizer
  #        S/(S+E+hI+lI+uhI+ulI+R+Im)-> E",                         # Mixing effect, excludes pI, H, cumI, and M since these compartments don't mix

  # Transitions
  transitions <- c(
    "@ -> mu*(S+E+hI+lI+uhI+ulI+R+Im) -> S",                                        # natural birth rate
    "R -> reSuscepRate*tempImmPeriod*R -> S",                           # Recovereds who become susceptible again
    "S -> (betaI*((1/phi)*(spreadRatio*hI + (1/spreadRatio)*lI))+betaP*pI+betaU*((1/phi)*(spreadRatio*uhI+(1/spreadRatio)*ulI)))*season*betaRandomizer*S/(S+E+hI+lI+uhI+ulI+R+Im)-> E", # Exposures - see above
    "E -> highSpreadProp*(1-isoRate)*exposedPeriod*E -> hI + cumI",     # Development of disease among non-isolated exposed high spreaders
    "E -> (1-highSpreadProp)*(1-isoRate)*exposedPeriod*E -> lI + cumI", # Development of disease among non-isolated exposed low spreaders
    ...
    "hI -> monitoringSuccess*(1-hospRatePost)*initInfectiousPeriod*hI -> pI",              # Transition from high spreader Infectious to Post-Infectious because of symptom monitoring
    "lI -> monitoringSuccess*(1-hospRatePost)*initInfectiousPeriod*lI -> pI",              # Transition from low spreader Infectious to Post-Infectious because of symptom monitoring
    "hI -> monitoringSuccess*hospRatePost*initInfectiousPeriod*hI -> H",
    "lI -> monitoringSuccess*hospRatePost*initInfectiousPeriod*lI -> H",
    "hI -> (1-monitoringSuccess)*initInfectiousPeriod*hI -> uhI",              # Transition to high spread unknown-infectious if not identified by symptoms/testing
    "lI -> (1-monitoringSuccess)*initInfectiousPeriod*lI -> ulI",              # Transition to low spread unknown-infectious if not identified by symptoms/testing
    ...
    "uhI -> (1-nonHospDeathRate)*unknownInfectiousPeriod*uhI -> R",     # Recovery among high spread unknown-infectious
    "ulI -> (1-nonHospDeathRate)*unknownInfectiousPeriod*ulI -> R",     # Recovery among low spread unknown-infectious
    "uhI -> nonHospDeathRate*unknownInfectiousPeriod*uhI -> M",         # Deaths among non-hospitalized COVID patients (high spread unknown-infectious)
    "ulI -> nonHospDeathRate*unknownInfectiousPeriod*ulI -> M",         # Deaths among non-hospitalized COVID patients (low spread unknown-infectious)
    ...
    "hI -> nu*hI -> @",                                                 # Non-Covid deaths among initial high spread infectious
    "lI -> nu*lI -> @",                                                 # Non-Covid deaths among initial low spread infectious
    "pI -> nu*pI -> @",                                                 # Non-Covid deaths among post infectious
    "uhI -> nu*uhI -> @",                                               # Non-Covid deaths among unknown high spread infectious
    "ulI -> nu*ulI -> @",                                               # Non-Covid deaths among unknown low spread infectious
  ...
  u0 <- data.frame(
    ...
    hI = round(I0*highSpreadProp,0),
    lI = round(I0*(1-highSpreadProp),0),
    uhI = rep(round(uI0Pop*highSpreadProp,0),NnumTrials),
    ulI = rep(round(uI0Pop*(1-highSpreadProp),0),NnumTrials),
  ...
  E <- cbind(diag(length(compartments)),c(rep(1,length(compartments)-xCompt),rep(0,xCompt)))
  ...
  if(is.null(paraEventsDF)) {
    ...
    parachuteSelect <- lapply(numParachuteList, function(x) ups(2,x,prob = c(highSpreadProp,1-highSpreadProp),replace=TRUE))
    parachuteSelect <- unlist(lapply(parachuteSelect, function(x) compartments[which(compartments %in% c("hI","lI"))][x]))
  ...
  if(is.null(massEntryEventsDF)) {
    ...
    if(maxMassEntryNodes > 0) {
      ...
      massEntryCumI <- massEntryEvents[which(massEntryEvents$select %in% which(compartments %in% c("hI","lI","uhI","ulI"))),]
  ...
  if(max(unlist(numParachuteList)) > 0 | !is.null(paraEventsDF)) { allEvents <- rbind(allEvents,parachuteEvents) }
  if(nrow(transferEvents)>0 | !is.null(transferEventsDF)) { allEvents <- rbind(allEvents,transferEvents) }
  if(maxMassEntryNodes > 0 | !is.null(massEntryEventsDF)) { allEvents <- rbind(allEvents,massEntryEvents) }
  if(length(superInfections | !is.null(superEventsDF))>0) { allEvents <- rbind(allEvents,superEvents)}
