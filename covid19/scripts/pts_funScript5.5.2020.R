...
pts_funScript <- function(
  ...
  pts_fun <- paste0(
    ...
    const double pdCounter_0 = v[11]; // Counter for pdDecay
    ...
    if(pdDecay > 0 && kbPhase == 3) {
      ...
      if(prev <= maxPrev1) {
        ...
        if(prevDown10 > downDelay)
          v_new[11] = pdCounter_0 + 1; // Increases pdCounter by 1
          if(v_new[11] < pdDecay) {
            // decrease phiBaseline toward 1
            pdB = log(-v_new[11] + (pdDecay + 1)) /  log(pdDecay + 1);
          } else {
    ...
    v_new[10] = (gdata[0] / gdata[1]) * ldata[1] * v_new[7] / v_new[0];
