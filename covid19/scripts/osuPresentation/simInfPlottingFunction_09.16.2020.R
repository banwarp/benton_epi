# simInfPlottingFunction_09.16.2020.R

# changes from simInfPlottingFunction_08.08.2020.R
# added option for user-defined y axis limits
# reduced individual trajectory line thickness
# decreased size of title text
# added subtitle

# changes from simInfPlottingFunction_06.20.2020.R
# allows spread and individual trajectories to be plotted
...
simInfPlottingFunction <- function(
                                   ...
                                   twoPeriod=FALSE,              # time span is by hours
                                   ...
                                   subtitleString = NULL,     # plot parameter: Subtitle of plot
  ...
  # creating date list
  if(twoPeriod) {
    dateList <- seq(ISOdate(2020,1,1)+startDate*(24*60*60)-(11*60*60),ISOdate(2020,1,1)+startDate*(24*60*60)-(12*60*60)+max(tS)*60*60*12,12*3600)
  } else {
  ...
  if(allTraj){
    p <- p +
      geom_line(data=allTrajDF, aes(x=dateList,y=compartment,group=trial), size=.1, color= "black", alpha = 0.1) +
  ...
  if(plotRandomTrajs > 0) {
    ...
    p <- p + geom_line(data=randomTrajDF, aes(x=randomTrajDF$dateList,y=randomTrajDF$compartment, group=randomTrajDF$trial), color=colorVec, size=.5,alpha = .5)
  }
  ...
  if(includeMedian) {
    ...
    if(sumGroups) {
      p <- p + geom_line(data = trajDF, aes(x=dateList,y=medVal,color=medVar), size = 1.5)
    } else {
      p <- p + geom_line(data = trajDF, aes(x=dateList,y=medVal,color=nodeGroup), size = 1.5)
    }
  }

  p <- p +
    labs(title = titleString, subtitle = subtitleString, x = xString, y = yString, color=lString, caption = cString)+
